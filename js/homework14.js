...
const calculateTotalBalance = users => copyUsers.reduce((acc, item) => acc += item.balance, 0);
...
const getNamesSortedByFriendsCount = users => copyUsers.sort((a, b) => a.friends.length - b.friends.length).map(item => item.name);
